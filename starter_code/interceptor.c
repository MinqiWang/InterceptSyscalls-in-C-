...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	return table[reg.ax].f(reg); // calling the original syscall
}
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(cmd == REQUEST_SYSCALL_RELEASE){
		...
		spin_lock(&calltable_lock);
		set_addr_rw((unsigned long)sys_call_table);
		sys_call_table[syscall] = this_table.f;
		set_addr_ro((unsigned long)sys_call_table);
		spin_unlock(&calltable_lock);
		spin_unlock(&pidlist_lock);
	}
	if(cmd == REQUEST_STOP_MONITORING){
		...
		if(current_uid() != 0){ // non-root monitor
			if(pid == 0){ // stop monitoring all pids is not allowed except for root
				...
				else{ // the list is a "black-list", check if pid is NOT on the list, aka is monitored
					if(check_pid_monitored(syscall, pid)){
						return -EINVAL;
					}
					// stop monitoring for this pid
	...
	if(cmd == REQUEST_START_MONITORING){
		...
		if(current_uid() != 0){ // non-root monitor
			...
			if(pid != 0){
				...
				destroy_list(syscall); // empty the list, making it a "black-list"
				spin_unlock(&pidlist_lock);
			}
		}
	}
...
static void exit_function(void)
{
	...
	// Restore the original MY_CUSTOM_SYSCALL and exit_group system call back
